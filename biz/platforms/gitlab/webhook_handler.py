...
class MergeRequestHandler:
    ...
    def add_merge_request_labels(self, labels: list) -> None:
        """给 MR 添加 label。GitLab 13+ 会在项目内自动创建不存在的 label。"""
        if not labels:
            return
        url = urljoin(f"{self.gitlab_url}/",
        ...
        logger.debug(
            f"Add labels {labels} to MR {self.merge_request_iid}: {response.status_code}, {response.text}")
        if response.status_code == 200:
            logger.info(f"Labels {labels} successfully added to MR.")
        else:
            logger.error(f"Failed to add labels: {response.status_code}")
            logger.error(response.text)
