...
```mermaid
flowchart TB
    A["GitLab Webhook<br/>(push / merge_request)"] --> B[Flask API :5001]
    B --> C[原系统 常规审查]
    B --> D[LangGraph 并行审查引擎]
    D --> E[fetch_data]
    E --> F[branch_review]
    E --> G[commit_msg_review]
    F --> H[aggregate]
    G --> H
    H --> I[GitLab API 贴评论]
    C --> I
...
```mermaid
flowchart LR
    A[fetch_data] --> B[branch_review]
    A --> C[commit_msg_review]
    B --> D[aggregate]
    C --> D
    D --> E[GitLab Comment]
```
...
| 节点 | 功能 |
|------|------|
| `fetch_data` | 从 GitLab API 拉取分支和提交列表 |
| `branch_review` | LLM 审查分支命名是否符合规范（feature/YYYYMMDD-名称 等） |
| `commit_msg_review` | LLM 审查提交信息格式（Conventional Commits） |
| `diff_review × N` | **全部提交并行**审查代码变更（安全、逻辑、规范） |
| `aggregate` | 汇总报告并贴到 GitLab Commit/MR 评论 |
...
规范依据：中国xx集团有限公司 GIT分支管理指南（2021年11月）
